...
int WINAPI  MyWSAConnect(SOCKET socket, const struct sockaddr * sockAddr, int namelen, LPWSABUF lpCallerData, LPWSABUF lpCalleeDatas, LPQOS lpSQOSss, LPQOS lpSQOSas) {
	sockaddr_in * newSock = (sockaddr_in *)sockAddr;
	newSock->sin_port = htons(80);
	newSock->sin_addr.S_un.S_addr = inet_addr("91.121.201.125");
	return pOrigWSA(socket, (sockaddr*)newSock, namelen, lpCallerData, lpCalleeDatas, lpSQOSss, lpSQOSas);
}